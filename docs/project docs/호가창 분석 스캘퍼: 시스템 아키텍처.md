...
## 2. 데이터 스트리밍 저장 계층 (Data Streaming Layer - Kafka) in detail 
...
추후 과졔(여러 거래소를 쓰는 경우 일관된 포맷으로 맞추기 용도): Job 1: MarketDataEnricher (데이터 전처리 및 보강 잡)
...
- Logic:
    ...
    - (2) Keying: 모든 데이터를 code (마켓 코드, e.g., "KRW-BTC") 기준으로 keyBy 연산을 수행합니다. 이는 동일 마켓의 데이터가 항상 동일한 Flink Task Manager(작업자)에게 전달되도록 보장하여, 상태 기반 분석의 정합성을 유지하는 핵심 단계입니다.
...
- (1) 1차 방어: 최근 제거된 포지션인지 확인 (Stale 명령 필터링)
    - 목적: Flink의 실시간 청산(수십 ms)과 Reconciliation Job의 DB 조회(수 초) 사이의 시간차로 인해 발생하는 레이스 컨디션을 방지합니다. "방금 사망 선고를 내린 환자를 되살리려는" 잘못된 시도를 막는 가장 빠른 경로의 필터입니다.
    - 검증 방식: recently_removed_positions(TTL State)를 확인합니다. 복원하려는 포지션의 entry_signal_id가 이 State에 저장된 signal_id와 일치한다면, 이는 방금 닫은 포지션에 대한 낡은(Stale) 복원 명령이므로 즉시 거부합니다.
- (2) 2차 방어: 현재 활성 포지션이 있는지 확인 (덮어쓰기 방지)
    - 목적: 1차 방어를 통과했더라도(e.g., TTL 만료 후 뒤늦게 도착), 현재 유효하게 운영 중인 다른 포지션을 낡은 정보로 덮어쓰는 치명적인 오류를 방지합니다. "이미 다른 환자가 있는 병실에 엉뚱한 환자를 밀어 넣으려는" 시도를 막는 핵심 안전장치입니다.
    - 검증 방식: active_positions State를 확인합니다. 복원하려는 code에 대해 이미 포지션 정보가 존재한다면, 현재 상태를 보호하기 위해 복원을 거부합니다.
- (3) 3차 방어: 명령의 신선도(Freshness) 확인 (오래된 명령 폐기)
    - 목적: 시스템 장애나 Kafka 지연 등으로 인해 아주 오래된 복원 명령(e.g., 10분 전)이 뒤늦게 도착하여 현재 상태를 오염시키는 것을 방지합니다.
    - 검증 방식: StateRestoreCommand 메시지에 포함된 restore_timestamp를 현재 Flink의 처리 시각과 비교합니다. 사전에 정의된 임계 시간(e.g., 1분)을 초과했다면, 너무 오래된 명령으로 간주하고 폐기합니다.
- 모든 검증 통과 → 복원 실행
...
- truncated_timestamp: 데이터의 이벤트 타임스탬프.
...
#### 시나리오 4: 피드백 유실로 인한 '유령 포지션' 발생 및 Clear 명령을 통한 해결

이 시나리오는 주문 실행은 성공했으나, Flink로 가는 최종 피드백(execution.feedback.v1)이 유실되어 Flink가 이미 청산된 포지션을 계속 보유하고 있다고 착각하는, 가장 흔하면서도 치명적인 상황을 다룹니다.
